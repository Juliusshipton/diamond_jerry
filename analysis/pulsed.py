...
class NMRFit(PulsedFit):

    """Provides fits and plots for a NMR measurement."""

    measurement = Instance(nmr.NMR, factory=nmr.NMR)
    
    fit_result = Tuple()
    text = Str('')
    
    freq =  Float(0.)#Property( depends_on='fit_result', label='period' )
    linew = Float(0.)
    contrast = Float(0.) #Property( depends_on='fit_result', label='contrast' )
    q = Float(0.) #Property( depends_on='fit_result', label='q' )
    
    def __init__(self):
        super(NMRFit, self).__init__()
        self.on_trait_change(self.update_fit, 'spin_state', dispatch='ui')
        self.on_trait_change(self.update_plot_tau, 'measurement.frequencies', dispatch='ui')
        self.on_trait_change(self.update_plot_fit, 'fit_result', dispatch='ui')

    def update_fit(self):
        # try to compute new fit from spin_state and tau. If it fails, set result to NaNs
        try:
            fit_result = fitting.fit_multiple_lorentzian(self.measurement.frequencies, self.spin_state,1)
        except:
            fit_result = (np.NaN * np.zeros(3), np.NaN * np.zeros((3, 3)), np.NaN, np.NaN)

        bg,freq, linew, amp = fit_result[0:3]
        A = np.abs(amp / (np.pi * freqwidth))
        contrast = 100 * A /bg 
    ...
    plots = [{'data':('frequencies', 'spin_state'), 'color':'blue', 'name':'NMR'},
             {'data':('frequencies', 'fit'), 'color':'red', 'name':'cos fit'} ]
    
    line_data = Instance(ArrayPlotData, factory=ArrayPlotData, kw={'frequencies':np.array((0, 1)),
                                                                    'spin_state':np.array((0, 0)),
                                                                    'fit':np.array((0, 0))})
    ...
    def update_plot_tau(self):
        self.line_data.set_data('frequencies', self.measurement.frequencies)

    def update_plot_fit(self):
        if self.fit_result[0][0] is not np.NaN:
            self.line_data.set_data('fit', fitting.NLorentzians(*self.fit_result)(self.frequencies))        
    
    traits_view = View(title='NMR Fit',
                       )

    get_set_items = PulsedFit.get_set_items + ['fit_result', 'contrast', 'freq','linew']
...
class PulsedAnalyzerHandler(GetSetItemsHandler):
    ...
    def new_NMR_fit(self, info):
        info.object.fit = NMRFit()
                       ...
                       menubar=MenuBar(Menu(Action(action='save', name='Save (.pyd or .pys)'),
                                  ...
                                  #TODO            Action(action='save_all', name='Save Line Plot (.png) + .pys'),
                                         ...
                                         Menu(Action(action='new_t1_measurement', name='T1'),
                                              ...
                                              Action(action='new_rabiawg_measurement', name='Rabi AWG'),
                                              Action(action='new_rabi2awg_measurement', name='Rabi2 AWG'),
                                              Action(action='new_nmr_measurement', name='NMR'),
                                              Action(action='new_nmrawg_measurement', name='NMR AWG'),
                                              Action(action='new_nmr0awg_measurement', name='NMR 0 AWG'),
                                              Action(action='new_nr_measurement', name='NuclearRabi'),
                                              Action(action='new_nrawg_measurement', name='NuclearRabi AWG'),
                                              Action(action='new_nr0awg_measurement', name='NuclearRabi 0 AWG'),
                                         ...
                                         Menu(Action(action='new_pulsed_fit', name='Pulsed'),
                                              Action(action='new_NMR_fit', name='NMR'),
