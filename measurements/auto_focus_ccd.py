"""
Auto focus tool.
instead of tracking the x,y , we track the ccdxoff,ccdyoff.
instead of the grid, we get the whole image each time
better just ignore the z function but keep it here
keep the marker function
...
from measurements.grab_ccd import grab_ccd_fast
...
class AutoFocus( ManagedJob, GetSetItemsMixin ):
	...
	size_xy					= Range(low=0.1, high=10., value=1.5,	desc='Size of XY Scan',					label='Size XY [micron]',			mode='slider',	auto_set=False, enter_set=True)
	size_z					= Range(low=0.1, high=10., value=2.,	desc='Size of Z Scan',					label='Size Z [micron]',			mode='slider',	auto_set=False, enter_set=True)
	step_xy					= Range(low=0.01, high=10., value=0.1,	desc='Step of XY Scan',					label='Step XY [micron]',			mode='slider',	auto_set=False, enter_set=True)
	step_z					= Range(low=0.01, high=10., value=0.1,	desc='Step of Z Scan',					label='Step Z [micron]',			mode='slider',	auto_set=False, enter_set=True)
	seconds_per_point_xy	= Range(low=1e-3, high=10, value=0.05,	desc='Seconds per point for XY Scan',	label='seconds per point XY [s]',	mode='text',	auto_set=False, enter_set=True)
	seconds_per_point_z		= Range(low=1e-3, high=10, value=0.2,	desc='Seconds per point for Z Scan',	label='seconds per point Z	[s]',	 mode='text',	 auto_set=False, enter_set=True)
	...
	def __init__(self, odmr_ccd):#, confocal):
		super(AutoFocus, self).__init__()
		self.odmr_ccd = odmr_ccd
		# self.confocal = confocal
		self.on_trait_change(self.update_plot_fit_image, 'fitdata_xy', dispatch='ui')
	...
	def _periodic_focus_changed(self, new):
		...
		if new:
			self.cron_event = CronEvent(self.submit, False, min=range(0,60,self.focus_interval))
	...
	def fit_xy(self):
		if self.fit_method_xy == 'Maximum':
			...
			return fitting.twodim_gaussian_fit(self.data_xy)
	...
	def add_target(self, key, coordinates=None):
		if coordinates is None:
			self.odmr_ccd.cam.Open()
			self.odmr_ccd.cam.AcquisitionMode.SetValue('SingleFrame')
			self.odmr_ccd.cam.ExposureAuto.SetValue('Off')
			self.odmr_ccd.cam.TriggerSelector.SetValue('FrameStart')	   
			self.odmr_ccd.cam.TriggerMode.SetValue("On")
			self.odmr_ccd.cam.TriggerSource.SetValue("Line1")
			self.odmr_ccd.cam.TriggerActivation.SetValue('RisingEdge')
			self.odmr_ccd.cam.PixelFormat.SetValue(self.odmr_ccd.switch)
			self.odmr_ccd.cam.BinningVertical = self.odmr_ccd.switchBin
			self.odmr_ccd.cam.BinningHorizontal = self.odmr_ccd.switchBin
			self.odmr_ccd.cam.ExposureMode.SetValue('Timed')
			self.odmr_ccd.cam.ExposureTime = self.odmr_ccd.exposure
			self.data_xy = grab_ccd_fast(self.odmr_ccd.cam, self.odmr_ccd.exposure, self.odmr_ccd.ccdwait)[self.odmr_ccd.y_min_pixel:self.odmr_ccd.y_pixel,self.odmr_ccd.x_min_pixel:self.odmr_ccd.x_pixel]
			self.data_xy = self.data_xy[self.odmr_ccd.y_min_sum:self.odmr_ccd.y_sum, self.odmr_ccd.x_min_sum:self.odmr_ccd.x_sum]
			self.data_xy = np.array((self.data_xy-np.min(self.data_xy)), dtype=np.float32)/(np.max(self.data_xy)-np.min(self.data_xy))
			self.odmr_ccd.cam.Close()
			self.fit_method_xy = 'Gaussian'
			fitxy, self.fitdata_pos = self.fit_xy()
			coordinates = np.array((fitxy[0], fitxy[1]))#, self.confocal.z))
		# if self.targets == {}:
			# self.forget_drift()
	...
	def focus(self):
			"""
			get the ccd data and fit them with multi-dimension gaussian function
			"""
			self.odmr_ccd.cam.Open()
			self.odmr_ccd.cam.AcquisitionMode.SetValue('SingleFrame')
			self.odmr_ccd.cam.ExposureAuto.SetValue('Off')
			...
			self.fit_method_xy = 'Gaussian'
			fitxy, self.fitdata_pos = self.fit_xy()
			self.fitdata_xy = fitting.multivariate_gaussian_model(self.fitdata_pos)(fitxy)
			new_drift = (fitxy[0]-self.targets[self.current_target][0], fitxy[1]-self.targets[self.current_target][1])
			print((fitxy[0],fitxy[1]),new_drift)
			self.odmr_ccd.ccdxoff = int(new_drift[0]+self.odmr_ccd.ccdxoff)
			self.odmr_ccd.ccdyoff = int(new_drift[1]+self.odmr_ccd.ccdyoff)
			self.odmr_ccd.cam.Close()
			# self.odmr_ccd.ccd_single()
			
			'''
			below are for the z-scanner
			the dummies for x n y scans are not figured out yet
			'''
			...
			# if not threading.current_thread().stop_request.isSet():
				...
				# data_z = scanner.scanLine(Line, self.seconds_per_point_z)
			...
			return fitxy[0], fitxy[1]#xp, yp, zp
	...
	def _plot_fit_data_image_default(self):
	...
	def _figure_fit_image_default(self):
		plot = Plot(self.plot_fit_data_image, width=100, height=100, padding=8, padding_left=48, padding_bottom=32)
	...
	def update_plot_fit_image(self):
		self.plot_fit_data_image.set_data('image', self.fitdata_xy)
	...
	traits_view = View(VGroup(HGroup(Item('submit_button', show_label=False),
							  ...
							  VSplit(Group(VGroup(Item('figure_image', show_label=False, resizable=True),
													label='original image',
												),
											VGroup(Item('figure_fit_image', show_label=False, resizable=True),
													label='fitted image',
												),
									layout='tabbed'
									),
	...
	get_set_items=['confocal','targets','current_target','current_drift','drift','drift_time','periodic_focus',
				   ...
				   'data_xy', 'data_z', 'fitdata_xy', 'X', 'Y', 'Z', 'focus_interval' ]
