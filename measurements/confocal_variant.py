...
from chaco.api import HPlotContainer, Plot, CMapImagePlot, ArrayPlotData, DataRange1D,\
                      Spectral, jet, reverse, ColorBar, LinearMapper, DataLabel, PlotLabel      
import chaco.api
...
class Confocal( ManagedJob, GetSetItemsMixin ):
    ...
    scanz = Bool(False)
    z_range=Range(low=scanner.getZRange()[0], high=scanner.getZRange()[1], value=3., label='z_range', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%.2f'))
    z_step=Range(low=scanner.getZRange()[0], high=scanner.getZRange()[1], value=0.7, label='z_step', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%.2f'))
    ...
    def update_mesh(self):
        ...
        if (x2-x1) >= (y2-y1):
            ...
            self.Y = numpy.linspace(y1,y2,int(self.resolution*round((y2-y1),2)/round((x2-x1),2)))
        else:
            self.Y = numpy.linspace(y1,y2,self.resolution)
            self.X = numpy.linspace(x1,x2,int(self.resolution*round((x2-x1),2)/round((y2-y1),2)))
