...
class ODMR( ManagedJob, GetSetItemsMixin ):
    ...
    frequency_begin = Range(low=1,      high=20e9, value=2.82e9,    desc='Start Frequency [Hz]',    label='Begin [Hz]', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    ...
    fit_frequencies = Array(value=np.array((np.nan,)), label='frequencies [Hz]') 
    fit_line_width = Float(np.nan, label='line width [Hz]', editor=TextEditor(evaluate=float, format_str='%.3e')) 
    fit_contrast = Float(np.nan, label='contrast [%]', editor=TextEditor(evaluate=float, format_str='%.1f'))
    ...
    def _run(self):
        ...
        try:
            ...
            ha.MicrowaveB().initSweep( self.frequency, self.power*np.ones(self.frequency.shape))
    ...
    def update_fit(self):
        ...
        self.fit_contrast = -100*p[3::3].mean()/(np.pi*p[2::3].mean()*p[0])
    ...
    def _create_line_plot(self):
        ...
        line_label = PlotLabel(text='', hjustify='left', vjustify='bottom', position=[64,32])
