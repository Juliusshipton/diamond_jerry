...
class Target(HasTraits):
    ...
    MWLaser=Float
    MWWait=Float
    MWTime=Float(100.,label='ODMR/ODMRP integration time [s]')
    Perform=Bool
    

class AutoPanelODMR2( FreeJob,HasTraits):
    ...
    table_editor = TableEditor(columns = [ObjectColumn(name='ID',width=120),
                                          ...
                                          ObjectColumn(name='MWTime',format='%3.2f',width=50),
    ...
    def _ExportASCI_fired(self):
        ...
        Header=list(['ID','MWBegin','MWEnd','MWDelta','MWPulsed','MWLaser','MWWait'])
        #Header=list(['ID','MWBegin','MWEnd','MWDelta','MWTime'])
        for items in Header:
            f.write(items)
            f.write('\t')
        f.write('\n')   
        for target in self.SpotInfo:
            ...
            Info=list([str(target.ID),str(target.MWBegin),str(target.MWEnd),str(target.MWDelta),str(target.MWPulsed),str(target.MWLaser),str(target.MWWait)])
            #Info=list([str(target.ID),str(target.MWBegin),str(target.MWEnd),str(target.MWDelta),str(target.MWTime)])
            for items in Info:
                f.write(items)
                f.write('\t')
            f.write('\n') 
    ...
    #def __init__(self,auto_focus, confocal,odmr,odmrp,odmrp2):
        super(AutoPanelODMR2, self).__init__()     
        ...
        print "Welcome to AutoPanelODMR2! For the SC measurement, please keep the default mw power!!! (SMIQ = -18 dBm)"
                ...
                for Target in  self.SpotInfo:
                    ...
                    if (self.PerformODMRP and self.odmrp):
                        if Target.Perform and self.refocus(Target.ID):
                            ...
                            self.odmrp.stop_time=Target.MWTime
                            if self.MWUser == True:
                    ...
                    if (self.PerformODMR and self.odmr):
                        if Target.Perform and self.refocus(Target.ID):
                            ...
                            self.odmr.stop_time=Target.MWTime
                            if self.MWUser == True:
    ...
    traits_view = View(VGroup(HGroup(Item('start_button', show_label=False),
                              ...
                              HGroup(VGroup(HGroup(Item('ImportSpots'),
                                     ...
                                     Tabbed(VGroup(HGroup(Item('folderUser'),
                                                  ...
                                                  # HGroup(Item('PerformODMRP2', enabled_when='odmrp2 != None'),
                                                   ...
                                                   #HGroup(Item('ODMRPWaitTime', enabled_when='odmrp != None')
                                                   #       ),
                                                   ...
                                                   #HGroup(Item('ODMRWaitTime', enabled_when='odmr != None')
                                                   #       ), 
                                                   label='measure'
                                                   ),
                       ...
                       width=1024,height=768,title='AutoPanelODMR2', buttons=['OK','CANCEL'],
