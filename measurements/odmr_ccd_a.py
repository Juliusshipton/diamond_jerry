...
class CCDODMR_a(ManagedJob, GetSetItemsMixin):
    ...
    # x_min = Range(low= 0., high=1199., value= 325, desc='x_min', label='x_min', mode='text', auto_set=False, enter_set=True)
    # x_max = Range(low= 1., high=1200., value= 820, desc='x_max', label='x_max', mode='text', auto_set=False, enter_set=True)
    # y_min = Range(low= 0., high=1919., value= 588, desc='y_min', label='y_min', mode='text', auto_set=False, enter_set=True)
    # y_max = Range(low= 1., high=1920., value= 1140, desc='y_max', label='y_max', mode='text', auto_set=False, enter_set=True)
    x_min = Int(value=0, label='x_min')
    x_max = Int(value=1200, label='x_max')
    y_min = Int(value=0, label='y_min')
    y_max = Int(value=1920, label='y_max')
    ...
    # scanner position
    y = Range(low=y_min, high=y_max, value=0, desc='y [Pixel]', label='y [Pixel]', mode='slider')
    x = Range(low=x_min, high=x_max, value=0, desc='x [Pixel]', label='x [Pixel]', mode='slider')
    cursor = Instance( CursorTool2D )
    cursor_position = Property(depends_on=['x','y'])#,'z','constant_axis'])
    
    
    def __init__(self):
        super(CCDODMR_a, self).__init__()
        self._create_ccd_plot()
        self._create_line_plot()
        # self.y = Range(low=int(self.y_min), high=int(self.y_max), value=int((self.y_max+self.y_min)/2), desc='y [Pixel]', label='y [Pixel]', mode='slider')
        # self.x = Range(low=int(self.x_min), high=int(self.x_max), value=int((int(self.x_max)+int(self.x_min))/2), desc='x [Pixel]', label='x [Pixel]', mode='slider')
    ...
    def apply_parameters(self):
        ...
        if not self.keep_data or np.any(frequency != self.frequency):
            ...
            self.counts_ccd_data = np.zeros(shape=(len(self.frequency), self.x_max-self.x_min,self.y_max-self.y_min))#original:1200x1920
            self.run_time = 0.0
    ...
    def _run(self):
        ...
        try:
            ...
            self.counts_ccd = grab_ccd_triggered2(cam)[self.x_min:self.x_max,self.y_min:self.y_max]
            #np.savetxt('ccd_rescale_test.txt', self.counts_ccd)
            counts_ccd = np.zeros((n,self.x_max-self.x_min,self.y_max-self.y_min)) #original:1200x1920
            while self.run_time < self.stop_time:
                ...
                for i in range(n):
                    ...
                    counts_ccd[i] = grab_ccd_triggered2(cam)[self.x_min:self.x_max,self.y_min:self.y_max]#+np.random.normal(127, 9, size=(1200, 1920))#self.laser, self.exposure, self.wait)
                    if threading.currentThread().stop_request.isSet():
                        break
                    time.sleep(0.1)
                    

                    #np.savetxt('ccd_rescale_test.txt', counts_ccd[i])
                # if len(self.counts)!=len(self.frequency):
                    # print("Error in len(counts) != len(frequency)")

                if threading.currentThread().stop_request.isSet():
                    break
                #self.counts = self.counts_resolve2(self.counts_resolve2(self.counts_resolve2(self.counts, int(self.mix/3)),int(self.mix/2)),self.mix)
    ...
    def _create_ccd_plot(self):
        ccd_data = ArrayPlotData(image=np.zeros((self.x_max-self.x_min, self.y_max-self.y_min)))#original:1200x1920
        ccd_plot = Plot(ccd_data, width=500, height=500, resizable='hv', aspect_ratio=1.0,padding=8, padding_left=64, padding_bottom=32)
        ccd_plot.img_plot('image',  colormap=jet, xbounds=(self.x_min,self.x_max), ybounds=(self.y_min,self.y_max), name='image')
        image = ccd_plot.plots['image'][0]
        image.x_mapper.domain_limits = (0,self.x_max)
        image.y_mapper.domain_limits = (0,self.y_max)
    ...
    def _update_line_data_value(self):
        self.line_data.set_data('counts_ccd_line', self.counts_ccd_data[:,self.x-self.x_min,self.y-self.y_min])
    ...
    traits_view = View(VSplit(HGroup(Item('submit_button',show_label=False),
                              ...
                              VGroup(HGroup(Item('power', width= -40, enabled_when='state != "run"'),
                                            ...
                                            Item('x_min', width= -80, enabled_when='state != "run"'),
                                            Item('x_max', width= -80, enabled_when='state != "run"'),
                                            Item('y_min', width= -80, enabled_when='state != "run"'),
                                            Item('y_max', width= -80, enabled_when='state != "run"'),
    ...
    get_set_items = ['frequency', 'counts_ccd', 'counts_ccd_data', 'x_min', 'x_max', 'y_min', 'y_max',
