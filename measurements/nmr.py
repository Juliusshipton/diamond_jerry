...
class NMR(Pulsed):
    ...
    switch = Enum('rf_2', 'rf_1', desc='switch to use for different RF source', label='switch')
    rf_power = Range(low= -130., high=25., value= -2, desc='RF power', label='RF power [dBm]', mode='text', auto_set=False, enter_set=True)
    ...
    wait = Range(low=1., high=1.0e7, value=1.2e6, desc='wait [ns]', label='wait [ns]', mode='text', auto_set=False, enter_set=True)
