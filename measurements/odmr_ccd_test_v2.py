...
class CCDODMR_test2(ManagedJob, GetSetItemsMixin):
    ...
    x1 = Range(low=0, high=1199, value=x_min_pixel, desc='x1 [micron]', label='x1', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%.2f'))
    y1 = Range(low=0, high=1919, value=y_min_pixel, desc='y1 [micron]', label='y1', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%.2f'))
    x2 = Range(low=1, high=1199, value=x_pixel, desc='x2 [micron]', label='x2', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%.2f'))
    y2 = Range(low=1, high=1919, value=y_pixel, desc='y2 [micron]', label='y2', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%.2f'))
    ...
    def __init__(self):
        super(CCDODMR_test2, self).__init__()
    ...
    traits_view = View(VSplit(HGroup(Item('submit_button',show_label=False),
                             ...
                             menubar=MenuBar(Menu(Action(action='saveLinePlot', name='SaveLinePlot (.png)'),
                                              ...
                                              name='File')),
                       title='CCD ODMR test v2', width=900, height=500, buttons=[], resizable=True, handler=CCDODMRHandler
                       )
