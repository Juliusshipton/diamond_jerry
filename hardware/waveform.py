    ...
    def append(self, *wave, **kw):
        for i, w in enumerate(wave):
            ...
            if isinstance(w, IQWaveform):
                wave = list(wave[:i]) + [w['i'], w['q'],] + list(wave[min(i+1, len(wave)):])
        l = len(wave)
...
class Sliceform(Sequence): 
    ...
    def __init__(self, name, pulse_seq, offset=0, iq=True, file_type='PAT', sampling=1.0e9):
    ...
    def compile(self):
        ...
        for index, pulse in enumerate(pulse_seq):
            if isinstance(pulse, Idle) and pulse.duration - stub >= 256:
                if index_0 < index:
                    name = self.base_name + ''
                    # slice
                    if index_0 == 0:
                        ...
                        piece = [carry,] + pulse_seq[index_0:index]
