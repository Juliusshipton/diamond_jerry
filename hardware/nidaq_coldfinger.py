...
class MultiBoard( CounterBoard ):
    ...
    def WriteAO(self, data, start=False):
        CHK( dll.DAQmxWriteAnalogF64( self.AOTask,
                                      ctypes.c_int32(self._AOLength), start,
                                      ctypes.c_double(self._RWTimeout),
...
class AOBoard():
    ...
    def Write(self, data):
        CHK( dll.DAQmxWriteAnalogF64(self.Task, ctypes.c_long(1), 1, ctypes.c_double(1.0), DAQmx_Val_GroupByChannel, data.ctypes.data, ctypes.byref(self._Nwritten), None) )
...
class LegacyScanner():
    #TODO: fix this
    """Scanner based on old national cards.
    Timing is on software level, i.e., set single AO value, wait, read count, etc... 
    """

    def __init__(self, CounterIn, CounterOut, TickSource, AOChannels,
                 x_range, y_range, z_range):

        self.AOTask = AOTask(Channels=AOChannels, range=(0.0,10.0)) 
        self.CITask = CITask(device=CounterIn)
    ...
    def setTiming(self, SettlingTime, CountTime):
        ...
        self._f = f
        self._DutyCycle = DutyCycle
        self._delay_time = SettlingTime + CountTime


    def setx(self, x):
        """Move stage to x, y, z
        """
        self.AOTask.Write(self.PosToVolt((x, self.y)))

    def sety(self, y):
        """Move stage to x, y, z
        """
        self.AOTask.Write(self.PosToVolt((self.x, y)))

    def ScanLine(self, Line):
        """Perform a line scan.
        """

        data = numpy.empty((Line.shape[0],))

        for i, point in enumerate(Line):
            self.AOTask.Write( self.PosToVolt(point) )
            time.sleep(self._delay_time)
            data[i] = self.CITask.Read()

        return data * self._f / self._DutyCycle

    def setPosition(self, x, y):
        """Move stage to x, y, z"""
        self.AOTask.Write(self.PosToVolt((x, y)))
...
#        y = self.yRange
#        z = self.zRange
#        return numpy.vstack( ( 10.0 / (x[1]-x[0]) * (Pos[0]-x[0]),
#                               10.0 / (y[1]-y[0]) * (Pos[1]-y[0]),
#                               10.0 / (z[1]-z[0]) * (Pos[2]-z[0]) ) )
...
class SnomScanner( MultiBoard ):
    """Scanner based on newer national cards.
    Timing is on software hardware level.
    Provides control of motion speed. 
    """
    ...
    def setx(self, x, speed):
        """Move stage to x, y, z
        """
        d = x - self.x
        st, ct = self.getTiming()
        dT = st + ct
        N = abs(int(d / (speed * dT)))
        if N > 1:
            X = numpy.linspace(self.x, x, N)
            Y = self.y * numpy.ones(X.shape)
            Z = self.z * numpy.ones(X.shape)
            Line = numpy.vstack( (X, Y, Z) )       
            self.ScanLine(Line)
        else:
            if self.AOLength() != 1:
                self.setAOLength(1)
            self.WriteAO(self.PosToVolt((x, self.y)), start=True)
        self.x = x

    def sety(self, y, speed):
        """Move stage to x, y, z
        """
        d = y - self.y
        st, ct = self.getTiming()
        ...
        N = abs(int(d / (speed * dT)))
        if N > 1:
            Y = numpy.linspace(self.y, y, N)
            X = self.x * numpy.ones(Y.shape)
            Z = self.z * numpy.ones(Y.shape)
            Line = numpy.vstack( (X, Y, Z) )       
            self.ScanLine(Line)
        else:
            if self.AOLength() != 1:
                self.setAOLength(1)
            self.WriteAO(self.PosToVolt((self.x, y)), start=True)
        self.y = y

    def setz(self, z, speed):
        """Move stage to x, y, z
        """
        d = z - self.z
        st, ct = self.getTiming()
        ...
        N = abs(int(d / (speed * dT)))
        if N > 1:
            Z = numpy.linspace(self.z, z, N)
            X = self.x * numpy.ones(Z.shape)
            Y = self.y * numpy.ones(Z.shape)
            Line = numpy.vstack( (X, Y, Z) )       
#            self.ScanLine(Line)
        else:
            pass
...
#            self.WriteAO(self.PosToVolt((self.x, self.y, z)), start=True)
    ...
    def setPosition(self, x, y, z, speed):
        """Move stage to x, y, z"""
        dy = y - self.y
        dx = x - self.x
        d = (dx**2 + dy**2)**0.5
        ...
        if N > 1:
            ...
            if self.AOLength() != 1:
                self.setAOLength(1)
            self.WriteAO(self.PosToVolt((x, y)), start=True)
        self.x = x
        self.y = y
    ...
    def ScanLine(self, Line):
        ...
        # acquire line
        self.WriteAO( self.PosToVolt(Line[0:2,:]) )
...
class Scanner( MultiBoard ):
    ...
    def __init__(self, CounterIn, CounterOut, TickSource, AOChannels,
                 x_range, y_range, z_range, TriggerChannels=None):
        MultiBoard.__init__(self, CounterIn=CounterIn,
                                  CounterOut=CounterOut,
                                  TickSource=TickSource,
                                  AOChannels=AOChannels)
...
class AnalogOutSyncCount():
    ...
    def __init__(self, ao_chan, co_dev, ci_dev, ci_port, ao_range=(-10,10), duty_cycle=0.9):
...
class AOTask(object):
    ...
    def Write(self, data):
        if self.N < numpy.inf:
            CHK( dll.DAQmxWriteAnalogF64(self.Task, ctypes.c_long(self.N), 0, ctypes.c_double(self.write_timeout), DAQmx_Val_GroupByChannel, data.ctypes.data, ctypes.byref(self.Nwritten), None) )
        else:
            CHK( dll.DAQmxWriteAnalogF64(self.Task, ctypes.c_long(1), 1, ctypes.c_double(self.write_timeout), DAQmx_Val_GroupByChannel, data.ctypes.data, ctypes.byref(self.Nwritten), None) )
...
class DOTask(object):
    ...
    def Write(self, data):
        ...
        CHK( dll.DAQmxWriteDigitalLines(self.Task, ctypes.c_long(1), 1,  ctypes.c_double(self.write_timeout), 1, data.ctypes.data, ctypes.byref(self.Nwritten), None ) )
...
class AITask(object):
    ...
    def Read(self):
        CHK( dll.DAQmxReadAnalogF64(self.Task, ctypes.c_long(self.N), ctypes.c_double(self.read_timeout), DAQmx_Val_GroupByChannel, self.data.ctypes.data, ctypes.c_ulong(3*self.N), ctypes.byref(self.Nread), None) )
        return self.data
