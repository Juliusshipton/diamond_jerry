...
#        self.xem.UpdateWireIns()
    ...
    def convertSequenceToBinary(self,sequence,loop=True):
        ...
        blank = np.zeros(N_CHANNELS,dtype=long) # we will need this many times, so we create it once and copy from this
        pattern = blank.copy()
        index = 0
        for channels, time in sequence:
            ticks = long(round(time/dt)) # convert the time into an integer multiple of hardware time steps
            if ticks is 0:
                continue
    ...
    def runSequence(self,loop=True,triggered=False):
        """
        Output a pulse sequence.
...
if __name__ == '__main__':
    ...
    pg = PulseGenerator(serial='12520004J8', core='24x4')
    time.sleep(0.1)
    pg.checkUnderflow()
